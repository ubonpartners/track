...
#
# Usage:
#   bench/run_pipeline.sh --config bench/pair_log_config_v19_clean.yaml \
#                         --tag v19 \
...
FP_BOOST=1.0
SEED=0
EVAL_BASE=bench/eval_base_uc_v11.yaml
...
python -m bench.export_phase3 --in "$PHASE3_PT" --out "$PHASE3_BIN"
...
python -m bench.export_decoupled_head --in "$STATE_HEAD_PT" --out "$STATE_HEAD_BIN"
...
for variant in full match_only state_only; do
    ...
    python -m bench.eval_head_fitness \
