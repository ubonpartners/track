...
ml/trace_library/
  __init__.py
  README.md            ← this file
  runner.py            ← `python -m ml.trace_library.runner --head HEAD.pt`
  capture.py           ← `python -m ml.trace_library.capture --corpus C.npz --seq S --tid T --out trace.npz`
  traces/
...
python -m ml.trace_library.capture \
    --corpus ml/data/state_corpus_v15ld_val.npz \
    ...
    --out ml/trace_library/traces/mu_tp_collapse_occlusion.npz

# 2. Edit the meta JSON to add `asserts` (capture.py prints a stub set).

# 3. Run the runner against the head you want to test.
python -m ml.trace_library.runner --head ml/data/state_head_dc_v9.pt
```
