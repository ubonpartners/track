...
Inspect any artefact from the shell:
        ...
        python -m ml.util._artefact_meta --read /path/to/foo.{bin,pt,npz}
"""
...
def require_npz_meta(path: str) -> Dict[str, Any]:
    ...
    if meta is None:
        raise ValueError(
            ...
            f"with ml.util._artefact_meta --attach-meta <path> "
            f"--comment <note>.")
    return meta
...
def _main():
    ap = argparse.ArgumentParser(prog="ml.util._artefact_meta")
