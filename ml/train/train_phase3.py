...
``UP3P``/v1 format that ``ml.export.export_phase3`` writes.
...
End-to-end pipeline:
    ...
    2. ``python -m ml.data_prep.build_pair_dataset`` aggregates those into
       ``ml/data/pairs_{train,val}.npz`` + ``feature_norm.json``.
    3. ``python -m ml.train.train_phase3 --save ml/data/match_phase3.pt``
       (this file).
    4. ``python -m ml.export.export_phase3 --in match_phase3.pt --out match.bin``
...
def auc(scores: np.ndarray, labels: np.ndarray) -> float:
    """ROC-AUC by Mann-Whitney U; matches sklearn's roc_auc_score on tied
    scores. Inlined here so the trainer has no external bench-side dep."""
...
def main():
    ...
    p.add_argument("--data_dir", default="ml/data",
