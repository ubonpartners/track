...
ITER0_CONFIG=ml/configs/pair_log_config_iter0_noNN_jaad.yaml
EVAL_BASE=ml/configs/eval_base_bootstrap.yaml
...
require_iter_outputs() {
    local n="$1"
    local match="ml/data/iter${n}/nn_match_iter${n}.bin"
    local state="ml/data/iter${n}/state_head_iter${n}_pw0.5.bin"
...
if [[ "$START_AT" -le 1 && "$STOP_AFTER" -ge 1 ]]; then
    ...
    ml/orchestration/run_pipeline.sh \
        --config "$ITER0_CONFIG" \
        --tag iter1 \
        --eval-base "$EVAL_BASE" \
        2>&1 | tee -a "$LOG"
...
if [[ "$START_AT" -le 2 && "$STOP_AFTER" -ge 2 ]]; then
    ITER2_CONFIG=ml/configs/pair_log_config_iter2_iter1NN.yaml
    make_iter_config \
        ml/data/iter1/nn_match_iter1.bin \
        ml/data/iter1/state_head_iter1_pw0.5.bin \
...
if [[ "$START_AT" -le 3 && "$STOP_AFTER" -ge 3 ]]; then
    ITER3_CONFIG=ml/configs/pair_log_config_iter3_iter2NN.yaml
    make_iter_config \
        ml/data/iter2/nn_match_iter2.bin \
        ml/data/iter2/state_head_iter2_pw0.5.bin \
