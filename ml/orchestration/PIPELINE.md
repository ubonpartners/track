...
silently ignored at runtime. Inspect it with:
    ...
    python -m ml.util._artefact_meta --read /mldata/config/track/trackers/nn_state_v14_dc.bin
...
2. Aggregate into split-level training datasets:
       ...
       python -m ml.data_prep.build_pair_dataset \
           ...
           --analysis-yaml ml/pair_log_config.yaml \
           --split train \
           --out ml/data/pairs_train.npz
       # repeat for --split val, then --split test

   The train-split run also writes `ml/data/feature_norm.json`.

3. Train the two-tower model:

       python -m ml.train.train_phase3 \
           --save ml/data/match_phase3.pt
   ...
   *permissive* pair-log dir (`new_track_thr=0.05`,
   ...
   `--pair-log-dir` explicitly.**
       ...
       python -m ml.data_prep.build_state_corpus \
           ...
           --gt-config ml/configs/pair_log_config_v15_permissive.yaml \
           --label-driven \
           --phase3-model ml/data/phase3_v10_face.pt \
           --out ml/data/state_corpus_v18 \
   ...
   `python -m ml.util._artefact_meta --read ml/data/state_corpus_v18_train.npz`.

2. Train the decoupled GRU head:

       python -m ml.train.train_state_head_decoupled \
           --train ml/data/state_corpus_v18_train.npz \
           --val   ml/data/state_corpus_v18_val.npz \
...
3. Export to runtime binary:
       ...
       python -m ml.export.export_decoupled_head \
           --in  ml/data/state_head_dc_v15.pt \
...
1. **Round-trip check** — re-run the exporter on the active `.pt` to
   confirm bit-identical bytes (excluding the metadata trailer):
       ...
       python -m ml.export.export_decoupled_head \
           --in ml/data/state_head_dc_v14_oldtrainer.pt --out /tmp/v14_re.bin
...
Everything else has been moved to `ml/_archive/` (see its README).
