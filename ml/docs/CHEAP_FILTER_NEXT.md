...
- match NN (`ml/data/iter1_d05v2/nn_match.bin`) trained on iter0_noNN_jaad
  pair-log with the per-event delta filter + threshold-lower-bound gate;
- state head (`ml/data/iter1_d05v2/state_head_pw0.5.bin`) trained on the
...
All on `ml/data/iter1_d05v2/`. Phase 3 + state corpus are already built.
...
for pw in 0.3 0.4 0.5 0.6 0.7 1.0; do
  python -m ml.train.train_state_head_decoupled \
    ...
    --save  ml/data/iter1_d05v2/state_head_pw${pw}.pt \
    --epochs 16 --seed 0 --hidden 64 --pos-weight $pw \
    --comment "iter1_d05v2 pw=${pw}"
  python -m ml.export.export_decoupled_head \
    --in  ml/data/iter1_d05v2/state_head_pw${pw}.pt \
    --out ml/data/iter1_d05v2/state_head_pw${pw}.bin
...
for feat in <candidate list from B1>; do
  python -m ml.train.train_phase3 \
    --epochs 25 --data_dir ml/data/iter1_d05v2 \
    --save ml/data/iter1_d05v2_drop_${feat}/phase3.pt --seed 0 \
    --drop-features $feat \
...
**C4 — `ml/_archive/` is gone; sweep for stale path references**
`grep -rn "_archive" ml/ src/` and prune any leftover lookups.
...
3. Rebuild `ml/data/iter1_d05v2_jaadall/pairs_*.npz` with same δ=0.5
...
but oversampling JAAD pairs at `ml/data_prep/build_pair_dataset.py` time (e.g.
...
- All experiments operate on the `matching-cheap-filter` branches of `track`
  ...
  `ml/data/<tag>/` so the .bin .meta.json chain stays clean.
