...
`ml/data/pairs_val.npz` (2.36M val pairs, 14% positives).

Tool: `ml/analysis/permute_match_features.py`. For each (view, column) it
...
exported to `ml/data/nn_match_v11_time.bin`. v11's
...
## A5 outcome — face conf in state head: positive offline+online, can't ship yet

End-to-end plumbing of det_subbox_conf / track_subbox_conf / det_fiqa_score
into the state-head GRU input (V3 schema, in_dim 25 → 28). Same recipe
as v20 (h=64, pos_weight=0.5, 8 epochs).

C side (commit `ubon_cstuff:0bacb81`):
- nn_state V3 = 28-dim (V2 25 + 3 face features at slots 25-27).
- nn_state_load accepts {19, 25, 28}.
- utdet_t gains `last_prev_subbox_conf` (mirroring last_prev_det_conf).
- utrack_state_head_run computes (det_subbox_conf, track_subbox_conf,
  det_fiqa_score) from tdet and feeds them to the V3 builder.

Python side: build_state_corpus.py V3 dtype (3 new fields, populated at
all 4 example-emit sites — both extract paths × matched/unmatched).
train_state_head_decoupled.py `--with-face` flag. export_decoupled_head
accepts in_dim=28. pair_log_schema decode_records gains a v2 (152-byte)
backward-compat path so the older pair_log_v9_face corpus can still be
read for state-corpus rebuilds.

Two state-corpus regens for clean comparison:

  v21 = built from `pair_log_v14_subdiou` (newest, v3 trace records,
        308k train rows). Smaller and slightly drifted from shipped.
  v22 = built from `pair_log_v9_face` (the same source the shipped
        state_corpus_v18 used; v2 trace records, 445k train rows).
        Closer to shipped corpus.

Per-corpus head-to-head (3-run fitness on diverse-29):

| Recipe                              | fitness | mota   | fp_tr |
|-------------------------------------|---------|--------|------:|
| nn_state_v20_pw05 (shipped baseline)| 0.6391  | 0.6555 | 23.7  |
| v20base_on_v21corpus                | 0.5994  | 0.6097 | 12.3  |
| **v21_face**                        | 0.6172  | 0.6277 | 12.0  |
| v20base_on_v22corpus                | 0.4668  | 0.4715 |  3.7  |
| **v22_face**                        | 0.5390  | 0.5490 | 13.7  |

Within-corpus deltas (face vs no-face, same corpus):
- v21 corpus: face = **+0.0178** fitness
- v22 corpus: face = **+0.0722** fitness

Face features add real, repeatable, fitness-positive signal across both
fresh corpora. Bigger lift on v22 (where the no-face baseline is the
weakest — the face channel rescues a chunk of mota that the no-face
head was leaving on the table by being over-conservative).

**But both retrains regress vs shipped v20** by 0.04–0.17 fitness —
the same corpus-drift symptom seen in A4 (warped DIoU). The
regenerated state corpora produce worse heads than shipped, even
trained with identical recipe + args. Most likely culprits:

- v22 from pair_log_v9_face: trace records were generated by an older
  C runtime; the regenerated head is calibrated against a PRE-update
  pair_score distribution, but the live runtime now feeds a slightly
  different value. Small input shift × strong head = big behaviour
  change in the conservative direction.
- v21 from pair_log_v14_subdiou: smaller corpus + 2 missing scenes
  (PP22_00000 and one other) tilts the loss away from the shipped
  scenes mix.

**Decision**: v22_face does NOT ship today. Keep v20 in production. The
result establishes:

1. Face conf in the state head is a valid, positive feature — add
   it back into the next state-head retrain with confidence.
2. The corpus-drift problem is the gating issue: until we can
   regenerate a corpus that reproduces shipped v20 fitness with the
   v20 recipe, every retrain (face or not) starts from a worse
   baseline. Pursuing this is the highest-value next state-head task
   and probably also explains a chunk of the v14_subdiou and v11_time
   regressions.

The C-runtime V3 path is kept (additive, harmless for v20 — V2 models
still load with the new 28-dim builder, they just read the first 25
slots). A future fitness-aware retrain that resolves the corpus drift
can ship v22_face (or a successor) by flipping the yaml `nn_state_path`
without further C changes.
...
- Corpus regen: `ml/configs/pair_log_config_v14_subdiou.yaml` (same scenes
  ...
  to `ml/data/pairs_{train,val,test}_v14.npz` with
