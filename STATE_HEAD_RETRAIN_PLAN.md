...
the corpus generated by `ml/data_prep/build_state_corpus.py` produces a
...
- Add PP22 sequences to gt-config for corpus generation
  (`ml/configs/pair_log_config_v7.yaml`).
...
- Use existing `ml/train_state_head.py` with sample weights wired
  through.
...
- Read `ml/build_pair_corpus.py` (or whatever feeds match-cost NN
  training).
...
- Round-trip test in `ml/round_trip_corpus_runtime.py`: feeds a
  ...
  — same role `ml/round_trip_schema_test.py` plays for the
...
2. **Refactor** `ml/data_prep/build_state_corpus.py` for label-driven replay
...
4. **Train + export** `nn_state_v6.bin` using existing
   `ml/train_state_head.py` (sample-weight wiring deferred to
...
`ml/data_prep/build_state_corpus.py` now has `extract_sequence_label_driven`
...
**Corpus v7** (`ml/data/state_corpus_v7_*.npz`): label-driven replay
...
**Trainer** (`ml/train_state_head.py`): wired the new `weight` field
through to the BCE loss as a sample-weight factor. Fallback: corpora
without `weight` get all-1.0.

**Training run** (`ml/data/state_head_v8.pt`):
...
**Export** (`ml/data/state_head_v8.bin` →
...
using the existing `ml/data/phase3_e100.pt` model. Failed:
...
`ml/configs/pair_log_config_v7_joint.yaml`: same 178 sequences as v6, but
...
- Pair-corpus consolidated via `ml/data_prep/build_pair_dataset.py`:
  train 922k pairs / 138 scenes, val 1.01M / 33 scenes, test 227k / 4.

### Phase 2e.2 — Advanced aggregators (`ml/train_phase3_adv.py`)
...
five `.pt` checkpoints kept in `ml/data/phase3_v7_*.pt` for the
record.

### Phase 2e.3 — State-corpus v8 + state-head v13

`ml/data_prep/build_state_corpus.py` invoked with
`--phase3-model ml/data/phase3_v7_ema_fixed.pt`, label-driven
...
`ml/configs/pair_log_config_v8_lowthr.yaml`. Production v14 NNs in the
...
`ml/build_creation_corpus.py`: walks emitted tracks, labels each
...
`ml/train_creation_head.py`: small MLP (7 → 32 → 32 → 3),
...
- `ml/build_creation_corpus.py`
- `ml/train_creation_head.py`
- `ml/configs/pair_log_config_v8_lowthr.yaml`
- `ml/data/creation_corpus_v1_{train,val,test}.npz`
- `ml/data/creation_head_v{1,2}.pt`
...
- `ml/data/pairs_{train,val,test}_v8.npz`
- `ml/data/phase3_v8_ema_fixed.{pt,bin}`
- `ml/data/state_corpus_v9_{train,val,test}.npz`
- `ml/data/state_head_v15_p{0.5,1.0,1.5,2.0,3.0}.{pt,bin}`
...
`ml/configs/pair_log_config_v9_face.yaml`: same setup as v7_joint but with
...
`ml/train_phase3_adv.py` extended with `--with_face` flag that
...
- `ml/data/phase3_v9_{noface,face}.pt`
- `ml/configs/pair_log_config_v9_face.yaml`
...
- match-cost: `ml/data/phase3_v9_{face,noface}.pt`,
  `/mldata/config/track/trackers/nn_match_v9_face.bin`
- state head: `ml/data/state_head_v16_p{1.0,2.0,3.0}.pt`,
  `/mldata/config/track/trackers/nn_state_v16_p{1_0,2_0,3_0}.bin`
- creation head: `ml/data/creation_head_v3.pt`
- state corpora: `ml/data/state_corpus_v10_{train,val,test}.npz`
...
Code changes:
- `ml/data_prep/build_state_corpus.py`: +5 fields in EXAMPLE_DTYPE,
  per-track `match_score_history` accumulator
- `ml/train_state_head.py`: N_INPUT 32→37 (3+9+5+5+16), input
...
- `ml/data/state_head_v18_p{1.0,2.0,3.0}.pt`
- `ml/data/state_head_v18_p{1.0,2.0,3.0}.bin`
- `/mldata/config/track/trackers/nn_state_v18_p{1_0,2_0,3_0}.bin`
- `ml/data/state_corpus_v11_{train,val,test}.npz`

Python source changes (uncommitted, on `master`):
- `ml/data_prep/build_state_corpus.py`: +5 EXAMPLE_DTYPE fields, history
  accumulator, pre-update emit
- `ml/train_state_head.py`: N_INPUT 32→37, build_input_matrix
...
Added `--fitness-fp-boost` to `ml/train_state_head.py`:
...
c. **Long-term: replace per-head BCE with fitness-shaped loss.**
   ...
   assignment (groundwork already in `ml/assign_sim.py`).
...
- `ml/data/state_head_v20_fb{1.0,5.0,20.0}.{pt,bin}`
...
Built `ml/eval/eval_head_fitness.py` — a closed-loop fitness primitive
that runs the actual C tracker on a stratified 29-clip subset (4
MOT17, 3 MOT20, 5 UKof, 5 INof, 12 PP22 — see
`ml/eval/eval_subset_diverse.json`) under user's eval params
...
- `ml/eval/eval_subset_diverse.json` — stratified 29-clip subset
...
- `ml/eval/eval_head_fitness.py` — the closed-loop primitive. Takes a
  YAML config or `--state-bin` path and returns aggregate fitness.
- `ml/diagnose_auc_vs_fitness.py` — the diagnostic that produced
  the table in 20.1.
...
Implemented in `ml/train_state_head.py` behind `--track-loss-enable`:
...
`ml/eval/eval_head_fitness.py --workers 4` on the 29-clip diverse
...
`ml/train_kseed_fitpick.py` — train K seeds, run diverse-29
...
`ml/soft_phantom_proxy.py` is a Python-only first pass at this
...
(`ml/data/state_corpus_v9_train.npz`, May 7 12:13) was generated
*before* the history-aggregates code was added to
`ml/data_prep/build_state_corpus.py` (Phase 6 commit `bb2c126`, later that
...
`ml/data/state_corpus_v11_train.npz` (May 7 18:23) already
...
- `ml/eval/eval_head_fitness.py` should gain a defensive check that
...
`ml/train_state_head.py --filter-matched-only` invalidates
...
`ml/data/state_head_v30_filtered.bin` (37-dim, hidden=64,
...
- `ml/train_state_head.py`: +`--track-loss-enable`,
  `--lambda-phantom`, `--lambda-miss`, `--tracks-per-batch`
- `ml/train_kseed_fitpick.py`: K-seed wrapper with diverse-29
  fitness pick.
- v21 / v22 heads: `ml/data/state_head_{v21,v22}_*.{pt,bin}` —
  artefacts of the variance investigation; kept for reproducibility.
- v23 head: `ml/data/state_head_v23.{pt,bin,kseed.json}`.
...
experiments). The implementation in `ml/train_state_head.py` is
...
groundwork in `ml/assign_sim.py` is the natural starting point.
...
- v21 heads: `ml/data/state_head_v21_baseline{,_s1}.{pt,bin}`,
  `ml/data/state_head_v21_20c{,_light}.{pt,bin}` — kept as
...
- `ml/train_state_head.py`: +`--no-history`, +`--fitness-fp-boost`
- `ml/data_prep/build_state_corpus.py`: +`--fitness-fp-boost` (corpus-time
  variant of the same logic; not used in this round, kept for completeness)
- v19 heads: `ml/data/state_head_v19{,b}_fb{1.0,5.0,20.0}.{pt,bin}`
...
- `ml/data/state_head_v41.{pt,bin}` copied to
...
- `ml/data/state_head_v41.{pt,bin}` — the cemented head
...
- `ml/data_prep/build_state_corpus.py` — corpus_v14 with state-age replay
  (NOTE: producing regression — see Open Paths #1)
- `ml/train_state_head.py` — `--no-drop-heads` flag for 2-head training
- `ml/soup_state_head.py` — model-soup utility (proven not helpful
...
- `ml/train_bayesian_head.py` — kept; useful as a 3-head trainer
- `ml/export_bayesian_head.py` — kept; .pt → 4-head .bin translator
- `ml/eval_bayesian_head_offline.py` — kept; offline diagnostic
- `track/BAYESIAN_TRACKER.md` — design doc (still valid; needs sec on
  creation-frame AUC requirement before this is viable)
- `ml/data/bayes_head_v{1,2}.{pt,bin}` — prototype heads, not
  shippable
...
- `ml/train_state_head_gru.py`
- `ml/export_gru_state_head.py`
- `ml/eval_gru_head_offline.py`
- `ml/data/state_head_gru_v1.{pt,bin}`
...
1. **Permissive pair-log generation** —
   `ml/configs/pair_log_config_v15_permissive.yaml` with `new_track_thr=0.05`
   (was 0.3). Phase1 ran in 13 min over 178 scenes; 1 scene failed
   (PP22_00131) but val/test/most of train are clean.

2. **State corpus v15** —
   `ml/data/state_corpus_v15_{train,val,test}.npz`. New balance:
...
3. **Decoupled head** — `ml/train/train_state_head_decoupled.py`
...
4. **Single-track diagnostic** — `ml/analysis/infer_single_track.py`
...
5. **Cost-rule offline simulator** — `ml/eval/eval_decoupled_offline.py`
...
2. **Run real closed-loop bench** — once C runtime is wired, run
   `ml/eval/eval_head_fitness.py --subset full` and compare the actual
...
- `ml/configs/pair_log_config_v15_permissive.yaml`
- `ml/data/state_corpus_v15_{train,val,test}.npz`
- `ml/train/train_state_head_decoupled.py`
- `ml/analysis/infer_single_track.py`
- `ml/eval/eval_decoupled_offline.py`
- `ml/data/state_head_dc_v{1,2,3}.pt` — v1 rebalanced (under-conf),
  v2 no rebalance on corpus_v13 (over-conf), **v3 on corpus_v15** (best)
- `ml/data/state_head_dc_v3.bin` — v3 .bin v3 (n_outputs=3 GRU)
- `ml/export/export_decoupled_head.py` — .pt → .bin v3 export
...
- `ml/eval/eval_head_fitness.py` gained `--utrack-override KEY=VALUE`
...
- `ml/analysis/infer_single_track.py`
- `ml/eval/eval_decoupled_offline.py`
- `ml/export/export_decoupled_head.py`
- `ml/data/state_head_dc_v3.{pt,bin}` — best decoupled checkpoint
...
Fix in `ml/train/train_state_head_decoupled.py`:
...
- `ml/train/train_state_head_decoupled.py` — `compute_per_frame_labels`
- `ml/data/state_head_dc_v4.{pt,bin}` — per-frame-labelled head
- `ml/dump_dc_decisions.py` — single-clip per-track diagnostic
...
1. New `ml/cost_rule.py` — single source of truth for the
   formulas. Pure functions, no torch/numpy deps.
2. New `ml/test_cost_rule.py` — 18 unit tests covering:
...
- `ml/cost_rule.py` — new, 100 lines
- `ml/test_cost_rule.py` — new, 200 lines, 18 tests
...
`ml/train/train_state_head_decoupled.py` (`--label-mode match-fraction`):
...
- `ml/cost_rule.py` (new) — single-source arithmetic
- `ml/test_cost_rule.py` (new) — 18 unit tests
- `ml/calibrate_head.py` (new) — head calibration measurement
- `ml/measure_corpus_coverage.py` (new) — FP-track coverage check
- `ml/measure_ntt_curve.py` (new) — empirical NTT precision/recall
- `ml/train/train_state_head_decoupled.py` — `--label-mode match-fraction`
- `ml/export/export_decoupled_head.py` — accept in_dim ∈ {19, 20}
- `ml/eval/eval_head_fitness.py` — fix int-vs-float yaml override
...
**Files touched**: `ml/eval/eval_head_fitness.py` already documents the
...
Added `ml/trace_library/` per the user's "interesting traces" idea.
...
Seed traces (in `ml/trace_library/traces/`):
...
**Files**: `ml/trace_library/{__init__,runner,capture}.py`,
`ml/trace_library/README.md`,
`ml/trace_library/traces/{5 seed traces}.npz`.
...
`ml/test_no_demote.py` runs the offline cost-rule sim with two state
...
**Files**: `ml/test_no_demote.py` (new).

---

## 2026-05-09 — Bug 10: every Phase 26+ bench was in LEGACY mode

`ml/eval/eval_head_fitness.py` only sets `state_head_bayesian=true` when
...
- **A**: `ml/eval_decoupled_offline.cost_decision` used delta_promote
...
- **B**: `ml/trace_library/capture.py` used a raw `(seq, tid)` mask
...
**Files**: see `ml/trace_library/`, `ml/eval/eval_decoupled_offline.py`,
...
Built:
  ...
  - Python side: `ml/data_prep/build_state_corpus.py` adds 6 fields to
    ...
    `ml/analysis/infer_single_track.py --scene-override` for sensitivity
    testing.
...
`ml/data/state_head_dc_v14_oldtrainer.{pt,bin}`: the v14 ckpt and
...
deleted the `SceneStats` replay class from `ml/data_prep/build_state_corpus.py`
...
1. `547aad1` — schema-sentinel guard `ml/util/verify_tree_sentinels.py`
   + pre-commit hook at `ml/githooks/pre-commit`. Activate per-clone
   with `git config --local core.hooksPath ml/githooks`. Asserts the
...
3. `055c52e` — restored `SceneStats` replay class in
   `ml/data_prep/build_state_corpus.py` (Python port of `utrack.c` lines
