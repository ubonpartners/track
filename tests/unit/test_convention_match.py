...
def test_permissive_iou_vertical_truncation():
    ...
    assert eval_matching.permissive_iou(FULLBODY, VISIBLE) > 0.999
    assert eval_matching.permissive_iou(VISIBLE, FULLBODY) > 0.999
    # identical boxes are still ~1.0 (no inflation of a perfect match)
    assert abs(eval_matching.permissive_iou(FULLBODY, FULLBODY) - 1.0) < 1e-6


def test_permissive_iou_x_offset_stays_unmatched():
    # fully x-offset: zero horizontal overlap -> score 0
    offset = [0.55, 0.20, 0.65, 0.40]
    assert eval_matching.permissive_iou(FULLBODY, offset) < 1e-9
    ...
    assert eval_matching.permissive_iou(FULLBODY, partial) < 0.2
    # and permissive never scores below plain IoU
    assert (eval_matching.permissive_iou(FULLBODY, partial)
...
def test_permissive_iou_matrix_matches_scalar():
    ...
    M = eval_matching.permissive_iou_matrix(gts, dets)
    assert M.shape == (2, 3)
    for i, g in enumerate(gts):
        for j, d in enumerate(dets):
            assert abs(M[i, j] - eval_matching.permissive_iou(g, d)) < 1e-9
    assert M[0, 0] > 0.999
...
def test_compute_metrics_convention_permissive(tmp_path):
    ...
    strict = eval_metrics.compute_metrics(gt, test, convention_permissive=False,
    ...
    perm = eval_metrics.compute_metrics(gt, test, convention_permissive=True,
                              classes_for_det_map=None)
