...
### 3.1 Practical-reality variants (MB addition)

"Fixed 5 fps" is itself a cadence pattern whenever `src_fps/5` is not an
integer — and its exact shape depends on the decimator policy. Including
both policies as data variants lets the experiment adjudicate the
decimator change (cadence.md P0-5) empirically, with zero C changes:

| variant | construction | what it is |
|---|---|---|
| `Pgear` | simulate NEAREST-TO-GRID at 200 ms ticks over the true source grid; e.g. 24 fps → kept {0,5,10,14,19}/s = 208/208/167/208/208 ms | the proposed decimator's real output: true 5.0 fps average, gear-tooth unevenness (one short step in five). On 23.976 sources the NTSC 1001/1000 factor adds slow phase precession — included free via the movie clips. |
| `Pfpt`  | simulate FIRST-PAST-THRESHOLD (today's `time_delta >= 0.2`, last-run snaps to passing frame); 24 fps → uniform every-5th = 208 ms = **4.8 fps** | current code's reality: clean cadence but a RATE error. Deliberately breaks the same-average design (labeled as such); paired against `Pgear` on the same clips it answers "is the gear unevenness cheaper than the rate deficit?" — the decimator A/B. |

Both are generated by the builder *simulating the policy* against the
source frame grid (src_fps, target 5 fps, policy) rather than hand-coded
cycles, so any (source, target) combination is coverable — including the
bodycam case (7.5 fps from 24 fps → 133/133/167 ms gear) if phase 2 wants
it. These two variants are only meaningfully distinct on non-integer-ratio
sources, so they are built for the 24/23.976 clips and (as micro-unevenness
controls) one 29.97 clip; integer-ratio clips skip them.
...
G2 exercises the cadence-estimator tracking rate-of-change; Pgear/Pfpt
(§3.1) are the practical decimator-policy realities on non-integer-ratio
sources. Five synthetic variants × 30 clips + the practical pair on the
~5 applicable clips ≈ 160 eval clips ≈ one ~5-minute eval round.
