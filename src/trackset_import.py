...
def _remux_to_mp4(src, dst):
    ...
    count, near-lossless crf 18) only if the source codec can't be
    stored in mp4. Writes via a temp name so interrupted runs can't
    leave a plausible-looking partial mp4."""
    import subprocess
    tmp = dst + ".part.mp4"
    r = None
    for args in (["-c", "copy"],
