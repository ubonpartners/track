...
def _update_initial_parameters(param_names, param_initial, source_dict, logfile, source_label):
    for key, value in source_dict.items():
        if key in param_names:
            ...
            if logfile:
                search_log(logfile, f"Setting parameter {key} initial value to {value} from {source_label}")
...
def search_test(config,
    ...
    for i,p in enumerate(params):
        c=config["tests"][result_test_opt_key]
        if p in c["config"]:
            c["config"][p]=param_vec[i]
        elif p in c["config"]["utrack"]:
            c["config"]["utrack"][p]=param_vec[i]
        elif p in c["config"]["motiontrack"]:
            c["config"]["motiontrack"][p]=param_vec[i]
        else:
            assert False, f"Can't find param {p} in config"
    search_log(logfile, f"..... testing {param_vec}")
    ...
    if full_result is None:
        raise RuntimeError(f"No result found for test {result_test_opt_key} on dataset {result_dataset_opt_key}")
...
def search_track(yaml_file):
    ...
    _update_initial_parameters(param_names, param_initial, test_dict.get("utrack", {}), logfile, "utrack")
    _update_initial_parameters(param_names, param_initial, test_dict.get("motiontrack", {}), logfile, "motiontrack")
    ...
    score_best, best_full_result=search_test(config, param_names, param_initial,
                                             param_min, param_max, results,
                                             split=train_split, logfile=logfile,
    ...
    while True:
        ...
        score_up,full_result_up=search_test(config, param_names, vec_up,
                                            param_min, param_max, results,
                                            split=train_split, logfile=logfile,
                                            desc=f"search test it:{iter_count} search param {param_names[index]} up")
        score_down,full_result_down=search_test(config, param_names, vec_down,
                                                param_min, param_max, results,
                                                split=train_split, logfile=logfile,
            ...
            if improvement!=0:
                for i in range(len(param_names)):
                    if total_improvement[i]!=0:
                        search_log(logfile, f"\t\t{param_names[i]:25s} improvement {total_improvement[i]:8.5f} {(100*total_improvement[i]/improvement):4.1f}% Value {vec_best[i]}")
